  ...
  return (
    <div className={`rounded-lg border px-3 py-2 shadow-lg ${
      ...
      <p className={`text-sm font-semibold ${headingText(darkMode)}`}>
        {payload[0].value} <span className={`font-normal ${subtleText(darkMode)}`}>hired</span>
      </p>
    </div>
  );
};
...
export const HiringTrendChart = ({ data = [], darkMode = false }) => {
  ...
  return (
    <div className={`p-5 rounded-2xl border shadow-[0_4px_20px_rgba(0,0,0,0.02)] ${
      ...
      <h4 className={`text-xs font-bold uppercase tracking-wider mb-3 ${subtleText(darkMode)}`}>Hiring Trajectory</h4>
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Tooltip content={<CustomTooltip darkMode={darkMode} />} cursor={{ stroke: "#ff6d34", strokeWidth: 1, strokeDasharray: "4 4" }} />
          <Line type="monotone" dataKey="hired" stroke="#ff6d34" strokeWidth={2.5} dot={{ r: 4, fill: "#ff6d34", stroke: darkMode ? "#2D2D2D" : "#fff", strokeWidth: 2 }} activeDot={{ r: 6, fill: "#ff6d34", stroke: darkMode ? "#2D2D2D" : "#fff", strokeWidth: 2 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
