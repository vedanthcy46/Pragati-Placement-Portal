  ...
  return (
    <div className={`rounded-lg border px-3 py-2 shadow-lg ${
      ...
      <p className={`text-sm font-semibold ${headingText(darkMode)}`}>
        {payload[0].value} <span className={`font-normal ${subtleText(darkMode)}`}>students hired</span>
      </p>
    </div>
  );
};

export const MonthlyAnalyticsChart = ({ data = [], darkMode = false }) => {
  if (!data || data.length === 0) return <EmptyState message="No monthly data available" darkMode={darkMode} />;
  return (
    <div className={`p-5 rounded-2xl border shadow-[0_4px_20px_rgba(0,0,0,0.02)] ${
      ...
      <h4 className={`text-xs font-bold uppercase tracking-wider mb-3 ${subtleText(darkMode)}`}>Monthly Hiring</h4>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={data} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="hired" fill="#00bea3" radius={[6, 6, 0, 0]} maxBarSize={36} />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
