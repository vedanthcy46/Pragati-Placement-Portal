import { BATCHES as FALLBACK } from "../../constants/reportsConstants";
import { useCollegeFilterOptions } from "../../../shared/hooks/useCollegeFilterOptions";

export const BatchFilter = ({ value, onChange, darkMode }) => {
  const { options } = useCollegeFilterOptions();
  const list = options?.batches?.length ? options.batches : FALLBACK;

  return (
    <div className="flex flex-col space-y-1.5 w-full">
      <label className={`text-xs font-semibold uppercase tracking-wider ${darkMode ? 'text-gray-400' : 'text-slate-500'}`}>Batch Year</label>
      <select
        value={value}
